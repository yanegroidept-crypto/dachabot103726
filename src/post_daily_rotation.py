...
один 4-дневный цикл, чтобы освободить место для Эдика):
...
вычисляется как ((сегодня - CYCLE_START_DATE).days % 4).

Скрипт запускается ОДИН раз в день (одним workflow/cron) и сам решает,
какое действие сегодня нужно выполнить — это удобнее, чем держать 4
...
def get_cycle_day(today: datetime.date | None = None) -> int:
    """Возвращает индекс дня цикла 0..3 для переданной даты (по умолчанию — сегодня)."""
